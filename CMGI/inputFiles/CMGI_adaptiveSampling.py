...
for f in glob.glob('dists-cmgi/Cmgi*_dist1_LYS-GLU.npy'):
	print(f)
	x = np.load(f)
...
for init in inits:
	...
	round = trj.split('.')[0][-1]       
	print(round)
	top = 'MD-rwTop/' + trj.replace('.mdcrd','.prmtop').replace('raw_cmgi','cmgi')
	rawTrj = 'MD-rwTrj/' + trj
	frame = init.split()[1]
	newTop = 'cmgi'+str(count)+'_md'+this_round+'.prmtop'
	newrst = 'cmgi'+str(count)+'_md'+this_round+'.rst'
